...
#endif
#ifndef DEFAULT_POLL_CMD
#define DEFAULT_POLL_CMD     "Q"
#endif
#ifndef DEFAULT_LINE_TIMEOUT
#define DEFAULT_LINE_TIMEOUT 300
#endif
#ifndef DEFAULT_ZERO_CMD
...
bool extractAdIdentityValue(const char* raw, const char* prefix, char* out, size_t outSize) {
...
bool isBalanceIdentityCommand(const char* cmd) {
  if (!cmd || !*cmd) return false;
  return strcmp(cmd, "?ID") == 0 ||
         strcmp(cmd, "?SN") == 0 ||
         strcmp(cmd, "?TN") == 0 ||
         strcmp(cmd, "I10") == 0;
}
...
bool captureIdentityResponse(const char* line) {
  ...
  if (pendingIdentity == ID_TYPE || pendingIdentity == ID_SICS_TYPE ||
    ...
    if (pendingIdentity == ID_TYPE && extractAdIdentityValue(line, "TN", identitySource, sizeof(identitySource))) {
      rawIdentity = identitySource;
    }
    compactIdentityLine(rawIdentity, balanceType, sizeof(balanceType));
    identityTypeDone = true;
    char upper[64];
    ...
    char identitySource[64];
    const char* rawIdentity = line;
    if (pendingIdentity == ID_SERIAL && extractAdIdentityValue(line, "SN", identitySource, sizeof(identitySource))) {
      rawIdentity = identitySource;
    }
    compactIdentityLine(rawIdentity, balanceSerial, sizeof(balanceSerial));
    identitySerialDone = true;
  } else if (pendingIdentity == ID_BALANCE_ID || pendingIdentity == ID_BALANCE_ID_ALT) {
    char identitySource[64];
    const char* rawIdentity = line;
    if (pendingIdentity == ID_BALANCE_ID) {
      if (extractAdIdentityValue(line, "ID", identitySource, sizeof(identitySource))) {
        rawIdentity = identitySource;
      }
    } else if (pendingIdentity == ID_BALANCE_ID_ALT) {
...
void updateIdentityFromExplicitCommand(const char* cmd, const char* line) {
  if (!cmd || !line || !*line || isIdentityErrorLine(line)) return;

  if (strcmp(cmd, "?TN") == 0) {
    char identitySource[64];
    const char* rawIdentity = line;
    if (extractAdIdentityValue(line, "TN", identitySource, sizeof(identitySource))) {
      rawIdentity = identitySource;
    }
    compactIdentityLine(rawIdentity, balanceType, sizeof(balanceType));
    identityTypeDone = true;
    ...
    char identitySource[64];
    const char* rawIdentity = line;
    if (extractAdIdentityValue(line, "SN", identitySource, sizeof(identitySource))) {
      rawIdentity = identitySource;
    }
    compactIdentityLine(rawIdentity, balanceSerial, sizeof(balanceSerial));
    identitySerialDone = true;
    ...
    identityPrimaryIdTried = true;
    applyClientBrandMapping();
  } else if (strcmp(cmd, "I10") == 0) {
    char ident[32];
    if (!extractSicsIdentityValue(line, ident, sizeof(ident))) return;
    if (extractClientBalanceId(ident, balanceId, sizeof(balanceId))) {
      char id[20];
      extractClientBalanceId(balanceId, id, sizeof(id));
      strlcpy(balanceId, id, sizeof(balanceId));
    } else {
      strlcpy(balanceId, ident, sizeof(balanceId));
    }
    if (isAllZeroIdentity(balanceId)) strlcpy(balanceId, "?", sizeof(balanceId));
...
void onBalanceLine(const char* line) {
  ...
  if (state == N_QUERYING && isBalanceIdentityCommand(pendingCmd) && isLikelyWeightLine(line)) {
    return;
  }
  if (state == N_QUERYING && isBalanceIdentityCommand(pendingCmd)) {
    updateIdentityFromExplicitCommand(pendingCmd, line);
  }
